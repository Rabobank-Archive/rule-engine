...
import nl.rabobank.oss.rules.dsl.nl.grammar.{Aanwezigheid, Berekening}
import nl.rabobank.oss.rules.engine.FactEngine
import nl.rabobank.oss.rules.utils.PrettyPrinter
import nl.rabobank.oss.rules.finance.nl.{Bedrag, Per}
...
class ResultOfTest (description: String, tester: BerekeningenTester) extends Matchers {
...
class ResultOfGegeven(val context: Context, val description: String, val tester: BerekeningenTester) {
