...
sealed trait DslConditionComparators[T] {
  ...
  def is(value: Fact[T]): DslCondition = combineWith(c => lhsEvaluation(c) == value.toFunc(c))

  def >[A <: T : Ordering : ClassTag](value: A): DslCondition = combineWith(compareWithValue(value, implicitly[Ordering[A]].gt))
  def >[A <: T : Ordering : ClassTag](value: Fact[A]): DslCondition = combineWith(compareWithEvaluation(value.toFunc, implicitly[Ordering[A]].gt))
  def >=[A <: T : Ordering : ClassTag](value: A): DslCondition = combineWith(compareWithValue(value, implicitly[Ordering[A]].gteq))
  def >=[A <: T : Ordering : ClassTag](value: Fact[A]): DslCondition = combineWith(compareWithEvaluation(value.toFunc, implicitly[Ordering[A]].gteq))

  def <[A <: T : Ordering : ClassTag](value: A): DslCondition = combineWith(compareWithValue(value, implicitly[Ordering[A]].lt))
  def <[A <: T : Ordering : ClassTag](value: Fact[A]): DslCondition = combineWith(compareWithEvaluation(value.toFunc, implicitly[Ordering[A]].lt))
  def <=[A <: T : Ordering : ClassTag](value: A): DslCondition = combineWith(compareWithValue(value, implicitly[Ordering[A]].lteq))
  def <=[A <: T : Ordering : ClassTag](value: Fact[A]): DslCondition = combineWith(compareWithEvaluation(value.toFunc, implicitly[Ordering[A]].lteq))
...
case class DslConditionPart[T] private[grammar](oldPart: DslCondition, fact: Fact[T], combineMethod: ConditionFunction) extends DslConditionComparators[T] {
  ...
  override private[grammar] def lhsEvaluation: Evaluation[T] = fact.toFunc
